...
data_4$TXN_TYPE <- data_4$Transaction.Type
...
data_pivot <- data_2[c(2,3,4,5,6,7,8,9,10,13,14)]
...
p <- plot_ly(data_4, x =~TXN_TYPE ,y = ~Processing_Time, type = "box")
p
```
...
p <- plot_ly(data_4, x =~TXN_TYPE ,y = data_4$THROUGHPUT_TIME.hours., type = "box")
p
```
...
fit <- lm(EDIT_COUNT ~ Processing_Time, data = data_4)
...
```{r}
valueBox(cor(data_4$Processing_Time,data_4$EDIT_COUNT),
         icon = "fa-area-chart")
```
